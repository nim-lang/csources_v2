...
N_LIB_PRIVATE N_NIMCALL(NIM_BOOL, rawCreateDir__stdZprivateZosdirs_431)(NimStringV2 dir);
...
N_LIB_PRIVATE N_NIMCALL(void, rawRemoveDir__stdZprivateZosdirs_424)(NimStringV2 dir);
...
{nimErr_ = nimErrorFlag();
	...
	LA8_: ;
	{
		...
		raiseOSError__stdZoserrors_70(T11_, dir);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}
	LA1_: ;
	}BeforeRet_: ;
...
{nimErr_ = nimErrorFlag();
	result = (NIM_BOOL)0;
	T1_ = (NIM_BOOL)0;
	T1_ = rawCreateDir__stdZprivateZosdirs_431(dir);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	result = !(T1_);
	{
		if (!result) goto LA4_;
				...
				res = ((NI)0);
				{
					while (1) {
						...
						i = res;
						{
							...
							T50_ = substr__system_7548(dir, ((NI)0), i);
...
{nimErr_ = nimErrorFlag();
	{
		...
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		raiseOSError__stdZoserrors_70(T8_, dir);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}
	LA6_: ;
	}BeforeRet_: ;
...
{nimErr_ = nimErrorFlag();
	...
	rawRemoveDir__stdZprivateZosdirs_424(dir);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}BeforeRet_: ;
}
N_LIB_PRIVATE N_NIMCALL(void, noscopyDir)(NimStringV2 source, NimStringV2 dest) {
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	{
		...
		d = opendir(nimToCStringConv(source));
		{
			if (!(d == ((DIR*) NIM_NIL))) goto LA5_;
			{
				...
				T11_ = osLastError__stdZoserrors_73();
				if (NIM_UNLIKELY(*nimErr_)) goto LA2_;
				raiseOSError__stdZoserrors_70(T11_, source);
				if (NIM_UNLIKELY(*nimErr_)) goto LA2_;
			}
			LA9_: ;
		}
