...
{nimErr_ = nimErrorFlag();
	result = (NIM_BOOL)0;
	n = NIM_NIL;
	eqcopy___ast_3457(&n, orig__izaJC4gcZ9cS9cHr6Ojm7cwg);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	{
		while (1) {
			...
			case ((tyEnum_TNodeKind__ySM4MdHjvuRpNcphR1xnyQ)47):
			{
				...
				if (!(T9_)) goto LA10_;
				T9_ = ((*(*n)._kind_4.sym).owner == owner__DUyiZaZq84Fl2sgPkgTzWQ);
				LA10_: ;
				T8_ = T9_;
				if (!(T8_)) goto LA11_;
	...
	if (!(T17_)) goto LA18_;
	T17_ = ((*(*n)._kind_4.sym).owner == owner__DUyiZaZq84Fl2sgPkgTzWQ);
	LA18_: ;
	T16_ = T17_;
	if (!(T16_)) goto LA19_;
	T20_ = 0;
...
{nimErr_ = nimErrorFlag();
	result = NIM_NIL;
	eqcopy___ast_3457(&result, n__6ggT9cDM1W2jyMcQXjPXbwA);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	{
		while (1) {
			switch ((*result).kind) {
...
{nimErr_ = nimErrorFlag();
	...
	field_2 = NIM_NIL;
	eqcopy___ast_3457(&obj_2, obj__v1yFps9a9b8Ov5WAAe4MQO2g);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	{
		while (1) {
			switch ((*obj_2).kind) {
	...
	eqcopy___ast_3457(&field_2, field__lG4EDCjgbOlpm9a002WkgMw);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	{
		while (1) {
			switch ((*field_2).kind) {
