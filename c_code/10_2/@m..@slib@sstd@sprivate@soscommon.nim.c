...
{	result = (NIM_BOOL)0;
	...
	T1_ = (NIM_BOOL)0;
	T2_ = (int)0;
	T2_ = stat(nimToCStringConv(filename__jr1cGx9bv9bKesh89cHdBsY0w), (&res));
	T1_ = (((NI32)0) <= T2_);
	if (!(T1_)) goto LA3_;
...
N_LIB_PRIVATE N_NIMCALL(tyTuple__MuQpYKhupAt9a7EFKGkr9ayg, getSymlinkFileKind__stdZprivateZoscommon_26)(NimStringV2 path__tkKX9aGRRocCQjqu2B7C85g) {
	...
	result.Field1 = colontmp__2;
	{
		int T3_;
		T3_ = (int)0;
		T3_ = stat(nimToCStringConv(path__tkKX9aGRRocCQjqu2B7C85g), (&s));
...
N_LIB_PRIVATE N_NIMCALL(NIM_BOOL, nosdirExists)(NimStringV2 dir__xk554PB9aLmMcTN0DEiUcAg) {
	...
	T1_ = (NIM_BOOL)0;
	T2_ = (int)0;
	T2_ = stat(nimToCStringConv(dir__xk554PB9aLmMcTN0DEiUcAg), (&res));
	T1_ = (((NI32)0) <= T2_);
	if (!(T1_)) goto LA3_;
...
{nimErr_ = nimErrorFlag();
	...
	result = (T1_ == ((NI32)0));
	{
		...
		isAccessDeniedError = (err == EXDEV);
		{
			...
			raiseOSError__stdZoserrors_70(err, colontmpD_);
			if (NIM_UNLIKELY(*nimErr_)) goto LA10_;
			{
				LA10_:;
			}
