...
N_LIB_PRIVATE N_NIMCALL(void, parse__sourcemap_529)(NimStringV2 source, tyObject_SourceInfo__DBtwBgn5pKlzQgLzz9bnCGg* Result);
...
static N_INLINE(NI, parseUntil__pureZparseutils_801)(NimStringV2 s, NimStringV2* token, NimStringV2 until, NI start);
...
static N_INLINE(NI, skipWhitespace__pureZparseutils_760)(NimStringV2 s, NI start);
static N_INLINE(NI, skipWhitespace__pureZparseutils_130)(NIM_CHAR* s, NI sLen_0);
static N_INLINE(NI, skipUntil__pureZparseutils_774)(NimStringV2 s, tySet_tyChar__nmiMWKVIe46vacnhAFrQvw until, NI start);
	...
	currFile = TM__QvdP9aXHBxhK6gLRMIcAD6Q_3;
	{
		...
		eolpos = ((NI)0);
		{
			while (1) {
				...
				T34_ = substr__system_7560(source, first, T28_);
...
{nimErr_ = nimErrorFlag();
	...
	eqcopy___system_3543((&(*Result).sources), (*info).files);
	eqcopy___system_3543((&(*Result).names), (*info).names);
	prevFile = ((NI)0);
	prevLine = ((NI)0);
	prevName = ((NI)0);
	prevNimCol = ((NI)0);
...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___sourcemap_702)(tyObject_SourceInfo__DBtwBgn5pKlzQgLzz9bnCGg* dest) {
	...
	eqdestroy___system_3540((&(*dest).names));
	eqdestroy___system_3540((&(*dest).files));
}
...
{nimErr_ = nimErrorFlag();
	...
	parse__sourcemap_529(source, (&node));
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___sourcemap_717)(tyObject_SourceMap__zVPgRO1YHLSfITsOPFuiMA* dest) {
	eqdestroy___system_3540((&(*dest).sources));
	eqdestroy___system_3540((&(*dest).names));
