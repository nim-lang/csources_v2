...
N_LIB_PRIVATE N_NIMCALL(tyTuple__XOVT24GHXwqiEj5GCsEGCA, execCmdEx__pureZosproc_1610)(NimStringV2 command, tySet_tyEnum_ProcessOption__bnU6W8LhTMnT4JaSWtGlSA options, tyObject_StringTableObj__V5PVrt9bIxZEeV7lfvqqtNg* env, NimStringV2 workingDir, NimStringV2 input);
...
N_LIB_PRIVATE N_NIMCALL(void, quit__system_6986)(NimStringV2 errormsg, NI errorcode);
...
{nimErr_ = nimErrorFlag();
	colontmp_ = ((NI)0);
	{
		while (1) {
			...
			if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
			eqdestroy___system_3511((&(*dest).p->data[colontmp_].substitutions));
...
{nimErr_ = nimErrorFlag();
	...
	eqdestroy___system_3511((&(*dest).Sup.filenames.idxToFilename));
...
(*dest).descRst.len = (*src).descRst.len; (*dest).descRst.p = (*src).descRst.p;
	if ((*dest).substitutions.p != (*src).substitutions.p) {	eqdestroy___system_3511((&(*dest).substitutions));
	}
...
{nimErr_ = nimErrorFlag();
	...
	colontmp_ = ((NI)0);
	{
		while (1) {
			...
			if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
			eqcopy___system_3514((&(*dest).p->data[colontmp_].substitutions), src.p->data[colontmp_].substitutions);
...
{nimErr_ = nimErrorFlag();
	result.len = 0; result.p = NIM_NIL;
	colontmpD_.len = 0; colontmpD_.p = NIM_NIL;
	colontmpD_ = substr__system_7562(rstText, ((NI)2));
