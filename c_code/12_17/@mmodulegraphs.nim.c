...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	eqdestroy___lineinfos_514((&(*dest).emittedTypeInfo.data));
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	eqsink___lineinfos_2019((&(*result).emittedTypeInfo), T18_);
	}BeforeRet_: ;
}
...
{nimErr_ = nimErrorFlag();
	...
	T11_.uniqueName = uniqueModuleName__msgs_1591((*g).config, ((NI32) ((*m).position)));
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	{
		...
		path = toFilenameOption__msgs_641(conf, fileIdx, (*conf).filenameOption);
...
{nimErr_ = nimErrorFlag();
	{
		...
		mint = (*m).position;
		colontmpD_ = toFullPath__msgs_589((*g).config, ((NI32) (mint)));
