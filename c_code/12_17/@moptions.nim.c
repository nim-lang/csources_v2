...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___options_3149)(tyObject_ConfigRefcolonObjectType___2BjaRtGfBjUbX51p9a4g9a9cg* dest) {
	...
	eqdestroy___system_3511((&(*dest).commandArgs));
	eqdestroy___stdZassertions_13((&(*dest).commandLine));
	eqdestroy___system_3511((&(*dest).extraCmds));
	eqdestroy___system_3511((&(*dest).implicitImports));
	eqdestroy___system_3511((&(*dest).implicitIncludes));
	...
	eqdestroy___system_3511((&(*dest).cLinkedLibs));
	eqdestroy___system_3511((&(*dest).externalToLink));
	eqdestroy___stdZassertions_13((&(*dest).linkOptionsCmd));
	eqdestroy___system_3511((&(*dest).compileOptionsCmd));
...
{nimErr_ = nimErrorFlag();
	result.len = 0; result.p = NIM_NIL;
	{
		...
		if (!(T3_)) goto LA4_;
		T5_ = (NIM_CHAR)0;
		T5_ = X5BX5D___system_7039(path, ((NI)1));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		T3_ = ((NU8)(T5_) == (NU8)(47));
		LA4_: ;
		if (!T3_) goto LA6_;
		result = substr__system_7550(path, ((NI)0), (NI)(path.len - ((NI)2)));
	}
	goto LA1_;
	LA6_: ;
	{
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	{
		...
		result = substr__system_7550(s, start, (NI)(ende - ((NI)1)));
	}
	goto LA7_;
	LA9_: ;
	{
		result = substr__system_7562(s, start);
	}
	LA7_: ;
	}BeforeRet_: ;
	return result;
...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___modulegraphs_9583)(tyObject_SuggestcolonObjectType___fw9a9cWgWYzM4zKmnq3DGNfQ* dest) {
	eqdestroy___system_3511((&(*dest).qualifiedPath));
