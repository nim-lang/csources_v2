	...
	currFile = TM__QvdP9aXHBxhK6gLRMIcAD6Q_3;
	{
		...
		eolpos = ((NI)0);
		{
			while (1) {
				...
				T34_ = substr__system_7550(source, first, T28_);
...
{nimErr_ = nimErrorFlag();
	...
	eqcopy___system_3514((&(*Result).sources), (*info).files);
	eqcopy___system_3514((&(*Result).names), (*info).names);
	prevFile = ((NI)0);
	prevLine = ((NI)0);
	prevName = ((NI)0);
	prevNimCol = ((NI)0);
...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___sourcemap_702)(tyObject_SourceInfo__DBtwBgn5pKlzQgLzz9bnCGg* dest) {
	...
	eqdestroy___system_3511((&(*dest).names));
	eqdestroy___system_3511((&(*dest).files));
}
...
N_LIB_PRIVATE N_NIMCALL(void, eqdestroy___sourcemap_717)(tyObject_SourceMap__zVPgRO1YHLSfITsOPFuiMA* dest) {
	eqdestroy___system_3511((&(*dest).sources));
	eqdestroy___system_3511((&(*dest).names));
