...
N_LIB_PRIVATE N_NIMCALL(tyObject_StandardFormatSpecifier__NDU7AW5Eu6Q69c8cH6RDouQ, parseStandardFormatSpecifier__pureZstrformat_57)(NimStringV2 s, NI start, NIM_BOOL ignoreUnknownSuffix);
...
{nimErr_ = nimErrorFlag();
	...
	LA30_: ;
	parsedLength = parseSaturatedNatural__pureZparseutils_837(s, (&result.minimumWidth), i);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	i += parsedLength;
	{
		NIM_BOOL T34_;
		...
		if (!T34_) goto LA36_;
		i += ((NI)1);
		parsedLengthB = parseSaturatedNatural__pureZparseutils_837(s, (&result.precision), i);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		i += parsedLengthB;
	}
	goto LA32_;
...
appendString((&(*result)), colontmpD__2);
	...
	LA4_: ;
	spec = parseStandardFormatSpecifier__pureZstrformat_57(specifier, ((NI)0), NIM_FALSE);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	radix = ((NI)10);
	switch (((NU8)(spec.typ))) {
	case 120:
