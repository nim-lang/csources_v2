...
N_LIB_PRIVATE N_NIMCALL(tyObject_StandardFormatSpecifier__NDU7AW5Eu6Q69c8cH6RDouQ, parseStandardFormatSpecifier__pureZstrformat_57)(NimStringV2 s__TrkHz1ehuJs8WNUHcBWdwA, NI start__lQEKOK5Mu9b1QpTqft648Gg, NIM_BOOL ignoreUnknownSuffix__Gt7m0gUH5wuef9csNi7sxbA);
...
{nimErr_ = nimErrorFlag();
	...
	LA30_: ;
	parsedLength = parseSaturatedNatural__pureZparseutils_837(s__TrkHz1ehuJs8WNUHcBWdwA, (&result.minimumWidth), i);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	i += parsedLength;
	{
		NIM_BOOL T34_;
		...
		if (!T34_) goto LA36_;
		i += ((NI)1);
		parsedLengthB = parseSaturatedNatural__pureZparseutils_837(s__TrkHz1ehuJs8WNUHcBWdwA, (&result.precision), i);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		i += parsedLengthB;
	}
	goto LA32_;
...
appendString((&(*result__cf3mpYwrFFZaaa4W7Luvag)), colontmpD__2);
	...
	LA4_: ;
	spec = parseStandardFormatSpecifier__pureZstrformat_57(specifier__rKQhlkpwEmo7gb69cTyH5SA, ((NI)0), NIM_FALSE);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	radix = ((NI)10);
	switch (((NU8)(spec.typ))) {
	case 120:
...
{nimErr_ = nimErrorFlag();
	...
	colontmpD_.len = 0; colontmpD_.p = NIM_NIL;
	spec = parseStandardFormatSpecifier__pureZstrformat_57(specifier__puOkRMs0FuqvfViA0tgZnw, ((NI)0), NIM_FALSE);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
