...
static N_INLINE(void, writeLine__msgs_447)(FILE* f, NimStringV2* x, NI xLen_0);
...
N_LIB_PRIVATE N_NIMCALL(void, flushDot__msgs_21)(tyObject_ConfigRefcolonObjectType___2BjaRtGfBjUbX51p9a4g9a9cg* conf);
...
N_LIB_PRIVATE N_NIMCALL(void, genSuccessX__msgs_1594)(tyObject_ConfigRefcolonObjectType___2BjaRtGfBjUbX51p9a4g9a9cg* conf);
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	colontmpD__2 = dollar___options_5270((*conf).projectFull);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
...
{nimErr_ = nimErrorFlag();
	{
		...
		i_2 = ((NI)0);
		{
			while (1) {
				...
				i = (&x[i_2]);
				write__stdZsyncio_248(f, (*i));
				if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
				i_2 += ((NI)1);
			} LA3: ;
		}
	}
	write__stdZsyncio_248(f, TM__hz3sEVFduMjVvdBnJCwMHA_33);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}BeforeRet_: ;
}
static N_INLINE(NIM_BOOL, isFilled__pureZcollectionsZtables_33)(NI hcode) {
		...
		L = T4_;
		{
			while (1) {
				...
				m = (&(*g).ifaces.p->data[i]);
				{
					...
					colontmpD__2.len = 0; colontmpD__2.p = NIM_NIL;
					colontmpD__2 = toFullPath__msgs_577((*g).config, ((NI32) ((*(*m).module).position)));
				...
				while (1) {
					...
					h = res;
					{
						...
						k = (&(*g).inclToMod.data.p->data[h].Field1);
						{
							...
							colontmpD__4 = toFullPath__msgs_577((*g).config, (*k));
...
{nimErr_ = nimErrorFlag();
	...
	colontmpD_ = getNimcacheDir__options_5047(conf);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
