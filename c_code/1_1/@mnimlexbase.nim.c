...
{nimErr_ = nimErrorFlag();
	...
	T6_ = NIM_NIL;
	T6_ = (*L).stream;
	charsRead = llStreamRead__llstream_218(T6_, ((void*) ((&(*L).buf[toCopy]))), (NI)((*L).sentinel + ((NI)1)));
		...
		s -= ((NI)1);
		{
			while (1) {
				...
				LA20_: ;
				{
					...
					T23_ = NIM_NIL;
					T23_ = (*L).stream;
					charsRead = llStreamRead__llstream_218(T23_, ((void*) ((&(*L).buf[oldBufLen]))), oldBufLen);
					if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
					{
...
NIM_BOOL* nimErr_;
{nimErr_ = nimErrorFlag();
	llStreamClose__llstream_144((*L).stream);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}BeforeRet_: ;
}
