...
static N_INLINE(void, pluseq___pureZstrutils_2354)(NF* x, NF y);
...
{nimErr_ = nimErrorFlag();
	...
	tos = (*prof).tos;
	eqcopy___options_811(&data, (*(*(*c).config).vmProfileData).data, NIM_FALSE);
	{
		while (1) {
			if (!!((tos == ((tyObject_TStackFrame__N30BaqTxdO9aOeBDi8omH3g*) NIM_NIL)))) goto LA3			;
			{
				...
				if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
				pluseq___pureZstrutils_2354((&(*T14_).time), ((NF)(tLeave) - (NF)((*prof).tEnter)));
				{
					tyObject_ProfileInfo__JQ9ap7uCnji5hTXw6N79bVDA* T19_;
					if (!(tos == (*prof).tos)) goto LA17_;
...
{nimErr_ = nimErrorFlag();
	result.len = 0; result.p = NIM_NIL;
	data = NIM_NIL;
	eqcopy___options_811(&data, (*pd).data, NIM_FALSE);
	echoBinSafe(TM__Ai9anooT0gIYA9c9b30PXqgdQ_2, 1);
	{
		...
		i_2 = ((NI)0);
		{
			while (1) {
				...
				if (NIM_UNLIKELY(*nimErr_)) goto LA5_;
				colontmpD__5 = toFileLineCol__msgs_658(conf, flMax);
				if (NIM_UNLIKELY(*nimErr_)) goto LA5_;
