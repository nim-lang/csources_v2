...
{nimErr_ = nimErrorFlag();
	...
	colontmpD_ = toFullPath__msgs_577(config, ((NI32) (position)));
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
...
{nimErr_ = nimErrorFlag();
	...
	filename = toFullPath__msgs_577(config, ((NI32) ((*m).position)));
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
...
{nimErr_ = nimErrorFlag();
	{
		...
		L = T2_;
		{
			while (1) {
				...
				colontmpD_ = origin;
				X5BX5Deq___lineinfos_597((&(*g).emittedTypeInfo), (*x), colontmpD_);
				if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
				i += ((NI)1);
			} LA4: ;
		}
