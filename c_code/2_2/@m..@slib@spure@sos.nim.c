...
N_LIB_PRIVATE N_NIMCALL(NimStringV2, getApplAux__pureZos_178)(NimStringV2 procPath);
...
{nimErr_ = nimErrorFlag();
	...
	colontmpD__2 = (int)0;
	{
		...
		int T6_;
		T3_ = (NIM_BOOL)0;
		T3_ = WIFSIGNALED__posixZposix_1064(status);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		if (!T3_) goto LA4_;
		T6_ = (int)0;
		T6_ = WTERMSIG__posixZposix_1055(status);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	...
	LA4_: ;
	{
		colontmpD__2 = WEXITSTATUS__posixZposix_1048(status);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		result = colontmpD__2;
	}
	LA1_: ;
