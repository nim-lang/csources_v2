...
{nimErr_ = nimErrorFlag();
	...
	(*result).kind = ((tyEnum_TLLStreamKind__9cSQ7roMMT510UcuCxRX05w)2);
	{
		NIM_BOOL T3_;
		T3_ = (NIM_BOOL)0;
		T3_ = open__stdZsyncio_406(&(*result).f, filename, mode, ((NI)-1));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		if (!!(T3_)) goto LA4_;
...
{nimErr_ = nimErrorFlag();
	...
	case ((tyEnum_TLLStreamKind__9cSQ7roMMT510UcuCxRX05w)2):
	{
		result = readLine__stdZsyncio_263((*s).f, line);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}
	break;
	case ((tyEnum_TLLStreamKind__9cSQ7roMMT510UcuCxRX05w)3):
	{
		result = readLine__stdZsyncio_263(stdin, line);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	}
	break;
	}
