...
N_LIB_PRIVATE N_NIMCALL(FILE*, open__stdZsyncio_450)(NimStringV2 filename, tyEnum_FileMode__4sJyy9aOSN8X63XdWXl5iDg mode, NI bufSize);
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	T1_ = (NI)0;
	assignIfDefault__main_703((&(*(*graph).config).errorMax), ((NI)IL64(9223372036854775807)), T1_);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	semanticPasses__main_40(graph);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
