...
appendString((&base), colontmpD__2);
		...
		{
			NimStringV2 T20_;
			if (!((*conf).backend == ((tyEnum_TBackend__s27DA9b8P4ksjjsndcLjR2w)3))) goto LA18_;
...
appendString((&T20_), TM__hz3sEVFduMjVvdBnJCwMHA_18);
		...
		{
			NimStringV2 blitTmp;
			if (!(((*conf).globalOptions &((NU64)1<<((NU)((((tyEnum_TGlobalOption__9bU4j0PZjdMiF7DRotXVQZw)7)))&63U)))!=0)) goto LA22_;
			blitTmp = base;
			base.len = 0; base.p = NIM_NIL;
		...
		LA22_: ;
		{
			...
			if (!(((*conf).globalOptions &((NU64)1<<((NU)((((tyEnum_TGlobalOption__9bU4j0PZjdMiF7DRotXVQZw)8)))&63U)))!=0)) goto LA25_;
		...
		LA25_: ;
		{
			...
			T31_ = rawNewString(base.len + OS__platform_520[((*conf).target.targetOS)- 1].Field10.len + 0);
appendString((&T31_), base);
appendString((&T31_), OS__platform_520[((*conf).target.targetOS)- 1].Field10);
			targetName = T31_;
		}
		LA16_: ;
...
{nimErr_ = nimErrorFlag();
	colontmpD_.len = 0; colontmpD_.p = NIM_NIL;
	colontmpD_ = jsonBuildInstructionsFile__extccomp_1624((*graph).config);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	runJsonBuildInstructions__extccomp_2779((*graph).config, colontmpD_);
	if (NIM_UNLIKELY(*nimErr_)) goto LA1_;
	{
		LA1_:;
	}
