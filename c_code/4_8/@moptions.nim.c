...
{nimErr_ = nimErrorFlag();
	result.len = 0; result.p = NIM_NIL;
	{
		...
		if (!(T3_)) goto LA4_;
		T5_ = (NIM_CHAR)0;
		T5_ = X5BX5D___system_7026(path, ((NI)1));
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		T3_ = ((NU8)(T5_) == (NU8)(47));
		LA4_: ;
		if (!T3_) goto LA6_;
		result = substr__system_7525(path, ((NI)0), (NI)(path.len - ((NI)2)));
	}
	goto LA1_;
	LA6_: ;
	{
...
{nimErr_ = nimErrorFlag();
	...
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	{
		...
		result = substr__system_7525(s, start, (NI)(ende - ((NI)1)));
	}
	goto LA7_;
	LA9_: ;
	{
		result = substr__system_7537(s, start);
	}
	LA7_: ;
	}BeforeRet_: ;
	return result;
