...
{nimErr_ = nimErrorFlag();
		...
		i_2 = ((NI)1);
		{
			while (1) {
				...
				} LA15: ;
				{
					...
					T30_ = 0;
					T31_ = (NIM_BOOL)0;
					T31_ = sameType__types_2000((*(*a).typ).sons.p->data[i], (*(*b).typ).sons.p->data[i], T30_);
					if (NIM_UNLIKELY(*nimErr_)) goto LA14_;
					if (!T31_) goto LA32_;
					{
						NIM_BOOL T36_;
	...
	}
	{
		if (!(result == ((tyEnum_MethodResult__mzqZ5KFgBLGzkX1JWKxmRw)2))) goto LA69_;
		{
			...
			T73_ = 0;
			T74_ = (NIM_BOOL)0;
			T74_ = sameTypeOrNil__types_2031((*(*a).typ).sons.p->data[((NI)0)], (*(*b).typ).sons.p->data[((NI)0)], T73_);
			if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
			if (!!(T74_)) goto LA75_;
			{
				NIM_BOOL T79_;
