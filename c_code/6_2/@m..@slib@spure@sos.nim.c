...
N_LIB_PRIVATE N_NIMCALL(NimStringV2, getApplFreebsd__pureZos_190)(void);
...
N_LIB_PRIVATE N_NIMCALL(NimStringV2, getApplHeuristic__pureZos_236)(void);
...
static N_INLINE(struct timespec, st_mtim__posixZposix_1047)(struct stat s__5n02p7GNFaoMLTvk1QYXJA);
...
{nimErr_ = nimErrorFlag();
	result.len = 0; result.p = NIM_NIL;
	result = getApplFreebsd__pureZos_190();
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	{
		NimStringV2 T5_;
		if (!(result.len == ((NI)0))) goto LA3_;
		T5_.len = 0; T5_.p = NIM_NIL;
		T5_ = getApplHeuristic__pureZos_236();
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
...
{nimErr_ = nimErrorFlag();
	...
	LA4_: ;
	T7_ = st_mtim__posixZposix_1047(res);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
